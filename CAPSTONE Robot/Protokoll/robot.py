    ...
    def getNextDirectionAstar(self,goal):#search(grid, init, goal, cost, heuristic):
                ...
                if x == goal[0] and y == goal[1]: # If goal
                    ...
                    for idx in range(len(self.movements)): # Loop to see each movement
                        ...
                        if x2 >= 0 and x2 < self.maze_dim and y2 >= 0 and y2 < self.maze_dim:
                            if Visited[self.Dimension_vector_mappingY[y2]][x2] == 0 and self.barriers[idx][self.Dimension_vector_mappingY[y],x] == 0 : # Each direction has its own barrier in the point of movement
                                g2 = g + 1
        ...
        while te != 0: # Loop until get from goal to initial position
            ...
            for idx in range(len(self.movements)):
                ...
                if x2 >= 0 and x2 < self.maze_dim and y2 >= 0 and y2 < self.maze_dim and self.barriers[Movment_to_save[idx]][self.Dimension_vector_mappingY[y2],x2] == 0 and Grid_values[self.Dimension_vector_mappingY[y2], x2] > -1:
